...
class TradeReportGenerator:
    ...
    def __init__(
        self, position_manager: PositionManager, capital_state: CapitalState | None = None
    ):
        """初始化報表生成器。
    ...
    def generate_detailed_report(self) -> str:
        ...
        # 初始化追蹤變數
        initial_capital = (
            self.capital_state.initial_capital
            if self.capital_state
            else self.position_manager.initial_cash
        )
        ...
        for trade in self.position_manager.trades:
            ...
            # 收付金額
            amount_str = (
                f"-{trade.amount:,.0f}" if trade.action == "買進" else f"+{trade.amount:,.0f}"
            )
        ...
        if self.position_manager.equity_curve and current_shares > 0:
            ...
            peak_col = self._pad_visual(
                f"{peak_price:,.2f}" if peak_price > 0 else "-", W_PEAK, ">"
            )
            change_col = self._pad_visual(change_str, W_CHANGE, ">")
            position_col = self._pad_visual(
                f"{position_count:.1f}/{num_positions}", W_POS, ">"
            )  # 使用實際份數
